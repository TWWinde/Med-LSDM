...
class SemanticGaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, seg, t, clip_denoised: bool, cond=None, cond_scale=1.):
        """
        Apply the model to get p(x_{t-1} | x_t), as well as a prediction of
        the initial x, x_0.
        ...
        :return: a dict with the following keys:
         ...
         """
        x_recon = self.predict_start_from_noise(
            x, t=t, noise=self.denoise_fn.forward_with_cond_scale(x, seg,  t, cond=cond, cond_scale=cond_scale))
        # elf.denoise_fn == Unet3D   x_recon == x_{0}
        if clip_denoised:
            s = 1.
            if self.use_dynamic_thres:
    ...
    @torch.inference_mode()  # add noise
    def p_sample(self, x, seg, t, cond=None, cond_scale=1., clip_denoised=True):
        """
        Sample x_{t-1} from the model at the given timestep. from noise
        ...
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(
            x=x, seg=seg, t=t, clip_denoised=clip_denoised, cond=cond, cond_scale=cond_scale)
        noise = torch.randn_like(x)
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b,
                                                      *((1,) * (len(x.shape) - 1)))
    ...
    def p_losses(self, x_start, seg,  t, cond=None, noise=None, **kwargs):
        b, c, f, h, w, device = *x_start.shape, x_start.device
        noise = default(noise, lambda: torch.randn_like(x_start))
        ...
        x_recon = self.denoise_fn(x_noisy, seg, t, cond=cond, **kwargs)  # predicted noise
